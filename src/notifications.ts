...
export class Notifications extends Model {
	...
	getFeed(roadmapId: string, cb: (err: any, results?: Array<INotification>) => void) {
		if (!roadmapId) {
			cb(new Error("no active roadmap"));
			return;
		}

		this.get("/notifications/" + roadmapId, (result: Array<INotification>) => {
			cb(null, result);
		}, (err: any) => {
			cb(err);
		});
	...
	newCount(roadmapId: string, cb: (err: any, count?: number) => void) {
		...
		this.get("/notifications/" + roadmapId + "/newcount", (count: number) => {
			cb(null, count);
		}, (err: any) => {
			cb(err);
		});
	...
	flagAsSeen(roadmapId: string, cb: (err: any, result?: boolean) => void) {
		this.put("/notifications/" + roadmapId + "/seen", null, (result: boolean) => {
			cb(null, result);
		}, (err: any) => {
			cb(err);
		});
